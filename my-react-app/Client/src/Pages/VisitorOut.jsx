...
const VisitorOut = () => {
  ...
  useEffect(() => {
    // Load visitors from localStorage and filter for "Out" status
    const storedVisitors = JSON.parse(localStorage.getItem('visitors')) || [];
    const visitorsOut = storedVisitors.filter(visitor => 
      visitor.status === 'Out' || visitor.status === 'Checked Out'
    );
    setVisitors(visitorsOut);
    setTotalVisitors(visitorsOut.length);
  }, []);
    ...
    <div className="visitor-container">
      ...
      <div className="registered-vehicles">
        ...
        <div className="vehicle-types">
          <button className="vehicle-type-btn" onClick={() => navigate("/visitor")}>All Visitors</button>
          <button className="vehicle-type-btn" onClick={() => navigate("/visitor-in")}>Currently In</button>
          <button className="vehicle-type-btn active">Checked Out</button>
        </div>
      </div>
      ...
      <div className="search-bar">
        ...
        <div className="visitor-types">
          <span>Total Checked Out: {visitors.length}</span>
        </div>
      </div>
        ...
        {currentVisitors.length > 0 ? (
          currentVisitors.map((visitor, index) => (
            <div key={visitor.id} className="visitor-row">
              ...
              <div className="visitor-cell">{visitor.time || 'N/A'}</div>
              <div className="visitor-cell">{visitor.checkOutTime || 'N/A'}</div>
              <div className="visitor-cell">
                {visitor.checkOutTime 
                  ? `${Math.floor((new Date(visitor.checkOutTime) - new Date(visitor.date)) / (1000 * 60))} mins` 
                  : 'N/A'}
              </div>
              <div className="visitor-cell">
          ...
          <div className="no-visitors">
            No visitors have checked out yet
          </div>
        )}
      </div>
